...
export class HudController {
  constructor({ document, eventTarget, THREE, CANNON, CustomEvent }) {
    this.THREE = THREE; this.CANNON = CANNON; this.CustomEvent = CustomEvent;
    ...
    this.dom.viewBtns.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        this.eventTarget.dispatchEvent(
          new this.CustomEvent("viewModeChanged", { detail: this.currentViewMode })
        );
      });
    });
  ...
  updateFlightData(speed, mach, altitude, vsi, throttle) {
    const data = formatFlightData({ speed, mach, altitude, verticalSpeed: vsi, throttle });
    const pairs = [["machDisplay", data.mach], ["speedDisplay", data.speed], ["altDisplay", data.altitude], ["vsiDisplay", data.verticalSpeed], ["throttleDisplay", data.throttle], ["hudSpeedOnScreen", data.raw.speed], ["hudAltOnScreen", data.raw.altitude], ["hudVsiOnScreen", data.raw.verticalSpeed]];
    for (const [key, value] of pairs) if (this.dom[key]) this.dom[key].innerText = value;
  }
  updateGear(gearDown) {
