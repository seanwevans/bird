export class WindVisualization {
  constructor(jetGroup, { THREE, random = Math.random }) {
    this.THREE = THREE;
    this.random = random;
  ...
  buildGeometry(jetGroup) {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      const r = this.TUNNEL_RADIUS * 0.95 * Math.sqrt(this.random());
      const theta = this.random() * Math.PI * 2;
      const x = r * Math.cos(theta);
      const y = r * Math.sin(theta);
      const z =
        this.PARTICLE_START_Z -
        this.random() * (this.PARTICLE_START_Z - this.PARTICLE_END_Z);
    ...
    this.windParticles = new this.THREE.LineSegments(
      this.lineGeometry,
      material,
    );
  ...
  update(velocityMag, simulatedMach, userOpacity, viewMode) {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      ...
      if (pz < this.PARTICLE_END_Z) {
        pz = this.PARTICLE_START_Z + this.random() * 5;
        px = this.originalX[i];
        py = this.originalY[i];
      }
